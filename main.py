...
def get_argparser():
    ...
    # Dataset Options
    parser.add_argument("--data_root", type=str, default='/home/shubhamp/Downloads/IDD_training/idd-lite/data',
...
def get_dataset(opts):
    ...
    if opts.dataset == 'cityscapes':
        ...
        train_dst = Cityscapes(root=opts.data_root, active_list = opts.active_list, split='train', transform=train_transform)
        val_dst = Cityscapes(root=opts.data_root, active_list = None, split='val', transform=val_transform)
        # print(len(train_dst))
        ...
        for i, (images, labels) in tqdm(enumerate(loader)):
            ...
            if opts.save_val_results:
                for i in range(len(images)):
                    ...
                    plt.close()
                    # file = open("results/"+str(img_id)+".txt", "w")
                    # file.write("%s \n" %(score))
                    # file.close()    
                    img_id += 1
...
def main():
    ...
    # ==========   Train Loop   ==========#
    vis_sample_id = np.random.choice(len(val_loader), opts.vis_num_samples, replace=False).tolist() if opts.enable_vis else None
